...
- [x] **Step 3：推基线分支到 origin**

```bash
git push -u origin fix/mypy-cleanup
```

- [x] **Step 4：创建 PR0 工作分支**
...
- [x] **Step 2：创建 root pyproject.toml**
...
- [x] **Step 3：精简 mypy.ini 为纯 mypy 配置**
...
- [x] **Step 1：检查 docs/technical/ 是否已有 35- 编号文件**
...
- [x] **Step 4：开 PR → fix/mypy-cleanup 基线分支**
...
- [x] **Step 1：在 backend 跑 mypy，按错误码筛选**
...
- [x] **Step 4：重复 Step 1-3 处理所有 Task 2.1 文件**

- [x] **Step 5：全 backend mypy 跑一次确认总数减少**
...
- [x] **Step 1：mypy 筛选 assignment 错误**
...
- [x] **Step 3：记录文件清单到 /tmp/mypy-pr3-files.txt**
...
- [x] **Step 2：逐个修复（典型模式：类型转换、None 检查、收紧类型）**
...
- [x] **Step 5：记录决策到 /tmp/mypy-pr4-decision.txt**
...
- [x] **Step 2：对每个文件改 `def execute` 签名匹配父类**
...
- [x] **Step 3-5：每文件 mypy + pytest 验证**

- [x] **Step 6：跑全 backend pytest（必须，BaseTool 影响所有子类）**
...
- [x] **Step 1：修改 `omni_desk_backend/smart_assistant/tools/base.py` 中 BaseTool.execute 抽象签名**
...
- [x] **Step 2：同步改 11 个子类（每文件 1 行）**

- [x] **Step 3-4：验证全 backend pytest + mypy**

- [x] **Step 5：拆 PR4a / PR4b**
...
- [x] **Step 2：记录文件清单到 /tmp/mypy-pr5-files.txt**
...
- [x] **Step 1：读当前 ci.yml typecheck 段**
...
- [x] **Step 2：删除 `continue-on-error: true`**

使用 Edit 工具移除该行（精确匹配文件内容）。

- [x] **Step 3：在 gate 任务 needs 列表加 typecheck**

```yaml
needs: [lint-backend, test-backend, lint-frontend, test-frontend, typecheck]
```

- [x] **Step 4：本地 yaml 语法验证**
