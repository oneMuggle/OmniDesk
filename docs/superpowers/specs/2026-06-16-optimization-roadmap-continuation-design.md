...
```
documents/views/documents.py:13:    pagination_class = None  (GeneratedDocumentViewSet)
users/views.py:185:                  pagination_class = None  (需读上下文判定)
permissions/views.py:29:             pagination_class = None  (权限下拉,可能保留)
events/views.py:764:                 pagination_class = None  (需读上下文判定)
```
...
| 事件 | 触发位置 | 现有状态 | 日志级别 |
|------|----------|----------|----------|
| 登录成功 | `users/views.py` 登录 view | 需补 | `info` |
| 登录失败(密码错/账号锁) | 同上 | 需补 | `warning` |
| JWT 刷新失败 | 后端中间件或 `users/views.py` 刷新端点 | 需补 | `warning` |
| 权限校验失败 | DRF permission class 或 `permissions/views.py` | 需补 | `warning` |
...
**范围:** `omni_desk_frontend/src/shared/api/*.js` (12 个非测试 .js 文件) → `.ts`/`.tsx`
