...
| 指标 | 目标 | 实测 | 状态 |
|---|---|---|---|
| 非流式 P95 | < 1.5s | 1059.7ms | ✅ |
| 非流式 P50 | — | 2.0ms | ✅ |
| 非流式 avg | — | 55.0ms | ✅ |
| 非流式 max | — | 1059.7ms | ✅ |
| 50 并发成功率 | 100% | 50/50 | ✅ |
| 缓存命中 TTFB | < 300ms(生产)/ < 500ms(本地) | 2.3ms | ✅ |

## P95 说明

P95 = 1059.7ms,接近 1.5s 阈值。主要构成:
- **首次请求开销**(约 1s):Django test client 初始化、orchestrator 首次导入、意图分类 LLM 调用
- **后续请求开销**(约 2ms):缓存命中、mock LLM 直接返回

生产环境(真实 PostgreSQL + Redis 缓存 + 连接池)下,P95 应远低于本地 :memory: SQLite 测试结果。  
本地测试的主要价值在于**回归检测** — 若后续优化导致 P95 显著上升,应立即排查。
