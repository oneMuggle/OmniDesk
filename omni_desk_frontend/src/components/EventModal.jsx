...
const EventModal = ({ isGuest: propsIsGuest = false,
  ...
  useEffect(() => {
    ...
    if (currentEvent?.extendedProps?.trialDetails) {
      ...
      scheduleApi.fetchTimeSlotsByTrial(trial.id)
        .then(slots => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (newTimeSlots.length > 0) {
          ...
          await scheduleApi.bulkCreateTimeSlots(formData.trial, newTimeSlots);
        } else {
        ...
        if (modifiedTimeSlots.length > 0) {
          ...
          await Promise.all(
            modifiedTimeSlots.map(slot => {
              return scheduleApi.updateTimeSlot(slot.id, slot);
            })
          );
        } else {
