...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    assigned_by = serializers.PrimaryKeyRelatedField(queryset=CustomUser.objects.all(), allow_null=True, required=False)
    assigned_by_username = serializers.CharField(source="assigned_by.username", read_only=True)
    personnel = serializers.StringRelatedField(read_only=True)
    personnel_id = serializers.PrimaryKeyRelatedField(
        queryset=Personnel.objects.all(), source="personnel", allow_null=True, required=False, write_only=True
    )
...
class UserAdminSerializer(serializers.ModelSerializer):
    ...
    personnel_id = serializers.PrimaryKeyRelatedField(
        queryset=Personnel.objects.all(), source="personnel", allow_null=True, required=False
    )
    phone_numbers = PhoneNumberSerializer(many=True, required=False)
