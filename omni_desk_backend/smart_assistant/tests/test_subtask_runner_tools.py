...
def test_confirmation_is_returned_without_reinjection_or_second_llm_call(monkeypatch):
    ...
    result = SubTaskRunner(router, EventBus(), 1, tool_registry=ConfirmationRegistry, user=user).run(
        make_subtask(), SharedContext("q")
    )
...
def test_tool_call_is_validated_executed_and_reinjected():
    ...
    bus = EventBus()
    result = SubTaskRunner(router, bus, 1, tool_registry=FakeRegistry, user=user).run(
        make_subtask(), SharedContext("q")
    )
...
def test_unknown_tool_is_rejected_without_execution_and_result_is_safe():
    ...
    result = SubTaskRunner(router, EventBus(), 1, tool_registry=FakeRegistry, user=user).run(
        make_subtask(), SharedContext("q")
    )
...
def test_notify_confirmed_tool_context_persists_sanitized_audit_event(db):
    ...
    values = {
        ...
        "title": "标题 alice@example.com",
        "content": "正文 13812345678",
        "scope": "self",
        "recipient_ids": [user.id],
        "operation_id": "op-1",
    }
    ...
    assert event.payload["sent_count"] == 0
