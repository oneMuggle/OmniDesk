...
class TestExtractUpdateParams(TestCase):
    def test_parses_full_update(self):
        with patch("smart_assistant.extractors.memo_update_extractor._call_update_llm") as mock_llm:
    ...
    def test_returns_none_when_target_is_non_string(self):
        with patch("smart_assistant.extractors.memo_update_extractor._call_update_llm") as mock_llm:
            mock_llm.return_value = '{"target_title": 123}'
            params = extract_update_params("改备忘")
        self.assertIsNone(params)

    def test_returns_none_when_all_changes_non_string(self):
        with patch("smart_assistant.extractors.memo_update_extractor._call_update_llm") as mock_llm:
            mock_llm.return_value = '{"target_title": "开会", "new_title": 123}'
            params = extract_update_params("把开会备忘的标题改掉")
        self.assertIsNone(params)
