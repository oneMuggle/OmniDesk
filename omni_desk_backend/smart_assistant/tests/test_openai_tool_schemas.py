...
def test_registry_get_openai_tools_returns_all_19():
    """ToolRegistry.get_openai_tools(user) 认证用户应拿到所有 19 个工具的 schema。
    ...
    User = get_user_model()
    user = User.objects.create_user(username="all19_user_t5", password="x")
...
def test_get_openai_tools_with_user_returns_all_for_authenticated_user():
    ...
    当前 19 个工具均 required_auth=True,故认证用户应拿到完整 19 个 schema。
    """
    from django.contrib.auth import get_user_model
...
def test_get_openai_tools_anonymous_user_filters_required_auth_tools():
    ...
    当前所有 19 个工具均 required_auth=True → 匿名用户应得到空列表。
    """
...
def test_get_openai_tools_signature_accepts_optional_user():
    ...
    工具(当前所有 19 个都 required_auth=True → 空列表)。这是 Task 5 引入
