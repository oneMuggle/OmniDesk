"""Tests for BaseTool OpenAI tool schema — 20 tools × strict JSON Schema validation.
...
def test_registry_assert_all_have_openai_schema_passes():
    """所有 20 个注册工具已实现 schema → registry lint 不抛错。"""
    from smart_assistant.tools.registry import ToolRegistry
...
def test_registry_get_openai_tools_returns_all_20():
    """ToolRegistry.get_openai_tools(user) 认证用户应拿到所有 20 个工具的 schema。
    ...
    assert len(schemas) == len(ALL_TOOL_CLASSES) == 20
...
def test_get_openai_tools_with_user_returns_all_for_authenticated_user():
    ...
    当前 20 个工具均 required_auth=True,故认证用户应拿到完整 20 个 schema。
    """
    from django.contrib.auth import get_user_model
    ...
    assert len(tools) == len(ALL_TOOL_CLASSES) == 20
...
def test_get_openai_tools_anonymous_user_filters_required_auth_tools():
    ...
    当前所有 20 个工具均 required_auth=True → 匿名用户应得到空列表。
    """
...
def test_get_openai_tools_signature_accepts_optional_user():
    ...
    工具(当前所有 20 个都 required_auth=True → 空列表)。这是 Task 5 引入
    ...
    # 当前 20 个都 required_auth=True → 空列表
    assert schemas == []
