...
class NewsArticleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if month:
            queryset = queryset.filter(
                publication_date__year=month.split("-")[0], publication_date__month=month.split("-")[1]
            )
        if type_id:
    ...
    def perform_create(self, serializer):
        serializer.save(
            personnel_id=self.request.data.get("personnel_id"), news_type_id=self.request.data.get("news_type_id")
        )
...
class NewsStatsView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        by_person_monthly = (
            NewsArticle.objects.annotate(month=TruncMonth("publication_date"))
            .values("personnel__username", "month")
            .annotate(count=Count("id"))
            .order_by("personnel__username", "month")
        )

        stats = {"total_articles": total_articles, "by_person": {}}
        ...
        for item in by_person_monthly:
            ...
            if person not in stats["by_person"]:
                stats["by_person"][person] = {"total": 0, "monthly": {}}
