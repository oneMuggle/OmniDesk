...
class DocumentModelsTest(TestCase):
    ...
    def test_generated_document_str(self):
        ...
        gen_doc = GeneratedDocument.objects.create(
            template=template, content="Generated content", generated_by=self.user
        )
...
class DocumentViewSetTests(APITestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username="api_doc_user", password="password", is_staff=True, is_superuser=True
        )
...
class BookImportViewTests(APITestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username="import_user", password="password", is_staff=True, is_superuser=True
        )
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
