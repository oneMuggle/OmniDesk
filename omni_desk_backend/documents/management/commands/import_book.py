...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--title",
            type=str,
            help="The title of the book. If not provided, attempts to extract from markdown.",
            default=None,
        )
    ...
    def handle(self, *args, **options):
        ...
        if not metadata["title"]:
            ...
            if title_match:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        "Could not determine book title. Please provide it via --title argument or ensure it's in the markdown."
                    )
                )
                return
        ...
        for chapter_order, chapter_data in enumerate(chapters_data):
            ...
            self.stdout.write(
                f"DEBUG: Processing chapter {chapter_order}: '{chapter_title}'. Raw content start: '{current_chapter_md_raw[:100]}...'"
            )

            # Create a new Markdown instance for each chapter to get its internal TOC
            chapter_md_processor = markdown.Markdown(extensions=md_extensions, extension_configs=md_extension_configs)
            ...
            for i, item in enumerate(chapter_heading_structure):
                ...
                if (
                    not cleaned_id
                ):  # If original ID from TOC resulted in empty after cleaning, try cleaning the source text
            ...
            if not chapter_data["is_preamble"]:
                ...
                if not found_matching_h1:
                    ...
                    chapter_heading_structure = [
                        item
                        for item in chapter_heading_structure
                        if not (item.get("level") == 1 and item.get("text") != normalized_chapter_title)
                    ]

                    # Sanitize the chapter_title for a valid ID
                    sanitized_id = clean_text_for_id(
                        chapter_title, chapter_order
                    )  # Use clean_text_for_id for ID generation

                    # Insert at the beginning to ensure it's the primary heading
                    chapter_heading_structure.insert(
                        0,
                        {
                            "level": 1,
                ...
                chapter_heading_structure.append({"level": 1, "id": "preamble", "text": "前言"})
            ...
            def replace_image_path(match):
                ...
                if image_abs_path.exists():
                    ...
                    self.stderr.write(
                        self.style.WARNING(f"    - Image not found (keeping original path): {original_path}")
                    )
                    return match.group(0)
            ...
            self.stdout.write(
                f"  - Imported chapter {chapter_order}: '{chapter_title}' (raw content start: '{current_chapter_md_raw[:50]}...')"
            )
        self.stdout.write(
            self.style.SUCCESS(f"Successfully imported {len(chapters_data)} chapters for the book '{book_obj.title}'.")
        )
