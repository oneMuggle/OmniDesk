...
from .models import Announcement, DocumentTemplate, PersonnelSequence, LeaderSequence, Position, Holiday

class AnnouncementViewSetTest(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.announcement = Announcement.objects.create(title="Test Announcement", content="Test content", author=self.admin_user)
        self.url = '/api/events/announcements/'
    ...
    def test_create_announcement(self):
        data = {'title': 'New Announcement', 'content': 'New content', 'author_id': self.admin_user.id}
        response = self.client.post(self.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Announcement.objects.count(), 2)

class DocumentTemplateViewSetTest(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.template = DocumentTemplate.objects.create(name="Test Template", experiment_type="chemical", template_file="templates/test.txt", owner=self.admin_user)
        self.url = '/api/events/document-templates/'
...
class SequenceViewSetsTest(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.p_sequence = PersonnelSequence.objects.create(name="Personnel Seq")
        self.p_sequence.personnel.add(self.person1, self.person2)
        self.p_sequence.sequence = [self.person1.id, self.person2.id]
        self.p_sequence.save()

        self.l_sequence = LeaderSequence.objects.create(name="Leader Seq")
        self.l_sequence.personnel.add(self.leader1, self.leader2)
        self.l_sequence.sequence = [self.leader1.id, self.leader2.id]
        self.l_sequence.save()

    def test_list_personnel_sequences(self):
        url = '/api/events/personnel-sequences/'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1)

    def test_list_leader_sequences(self):
        url = '/api/events/leader-sequences/'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1)

class PositionViewSetTest(BaseTestCase):
    def test_list_positions(self):
        url = '/api/events/positions/'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 2)

class HolidayViewSetTest(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.holiday = Holiday.objects.create(name="Test Holiday", start_date=date.today(), end_date=date.today() + timedelta(days=1))
        self.url = '/api/events/holidays/'
