...
from users.permissions import IsAdminOrManagerOrReadOnly, IsRequester, IsTargetPersonnel
...
# ---- SP2: ScheduleSwapRequest ViewSet(决策 1C:两人互认即生效) ----
...
class SwapRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """per-action 权限:行级对象级检查按 action 区分。

        - cancel:仅申请方 (IsRequester)
        - accept/reject:仅接收方 (IsTargetPersonnel)
        - list/retrieve/create:任何登录用户(行级过滤由 get_queryset 完成)
        """
    ...
    def get_serializer_class(self):
        if self.action == "list":
            from .serializers import SwapRequestListSerializer

            return SwapRequestListSerializer
        if self.action in ("retrieve", "accept", "reject", "cancel"):
            from .serializers import SwapRequestDetailSerializer, SwapRequestTargetActionSerializer

            if self.action in ("accept", "reject"):
                return SwapRequestTargetActionSerializer
            return SwapRequestDetailSerializer
        from .serializers import SwapRequestCreateSerializer
    ...
    def perform_create(self, serializer):
        ...
        try:
            with transaction.atomic():
                ...
                # L3 防护:Model.clean()(自换/过去日期/失效时间/非值班员)
                instance.full_clean()
        except DjangoValidationError as e:
            # 转为 DRF ValidationError → 自动返回 400
    ...
    def reject(self, request, pk=None):
        ...
        from events.models import ScheduleSwapAuditLog
...
class MyScheduleView(generics.ListAPIView):
    ...
    沿用 P2-3 MyPersonnelView 模式:无 personnel 关联 → 返回 []。
    """
