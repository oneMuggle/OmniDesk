...
from personnel.models import Position, Personnel, Contract, Education, WorkExperience, ProfessionalQualification, FamilyMember
from events.management.seeders.base import BaseSeeder

FIRST_NAMES = [
    "张伟", "李娜", "王芳", "刘洋", "陈静", "杨帆", "赵磊", "黄丽", "周涛", "吴敏",
    "徐强", "孙鹏", "马超", "朱琳", "胡斌", "郭艳", "何勇", "林峰", "高远", "罗军",
    "梁博", "宋佳", "谢宇", "韩雪", "唐亮", "许文", "邓辉", "萧然", "冯程", "曹杰",
]
DEPARTMENTS = [
    "研发部", "测试部", "质量部", "生产部", "工艺部", "设备部", "安全部", "综合管理部",
]
POSITIONS = [
    "研究员", "高级工程师", "工程师", "技师", "助理工程师", "项目经理", "质量工程师", "试验员",
]
    ...
    def seed(self):
        ...
        for i in range(count):
            ...
            personnel, _ = self.safe_get_or_create(
                ...
                name=name,
                defaults={
                    "id_card_number": f"110101{random.randint(19800101, 19991231):08d}{random.randint(1000, 9999):04d}"[:18],
                    "date_of_birth": birth,
            ...
            # 教育背景
            schools = ["清华大学", "北京大学", "北京航空航天大学", "北京理工大学", "中国科学院大学", "浙江大学", "上海交通大学", "复旦大学"]
            ...
            # 工作经历
            companies = ["华为技术有限公司", "中兴通讯", "中国电子科技集团", "航天科技集团", "中芯国际", "京东方", "比亚迪", "宁德时代"]
            ...
            if random.random() < 0.5:
                ...
                for _ in range(random.choice([1, 2])):
                    ...
                    self.safe_get_or_create(
                        ...
                        defaults={
                            "relationship": rel,
                            "id_card_number": f"110101{random.randint(19600101, 20001231):08d}{random.randint(1000, 9999):04d}"[:18],
