...
class MeetingRoomBooking(models.Model):
    meeting_room = models.ForeignKey(
        MeetingRoom, on_delete=models.CASCADE, related_name="bookings", verbose_name="会议室"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="meeting_bookings", verbose_name="预约用户"
    )
    ...
    def clean(self):
        ...
        conflicting_bookings = MeetingRoomBooking.objects.filter(
            meeting_room=self.meeting_room, start_time__lt=self.end_time, end_time__gt=self.start_time
        ).exclude(pk=self.pk)
...
class MeetingRoomMaintenance(models.Model):
    meeting_room = models.ForeignKey(
        MeetingRoom, on_delete=models.CASCADE, related_name="maintenance_periods", verbose_name="会议室"
    )
