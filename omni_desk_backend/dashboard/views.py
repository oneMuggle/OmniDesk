...
def dashboard_stats(request):
    ...
    today_schedules = Schedule.objects.filter(duty_date=date.today()).select_related("duty_person", "duty_leader")
    today_schedule_list = [
        {
    ...
    memos_due = (
        Memo.objects.filter(
            user=user, is_completed=False, reminder_time__isnull=False, reminder_time__lte=now + timedelta(days=7)
        )
        .order_by("reminder_time")[:5]
        .values("id", "title", "reminder_time", "is_completed")
    )
