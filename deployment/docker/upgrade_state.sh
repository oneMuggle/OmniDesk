...
# ─── 状态机定义 ───────────────────────────────────────────
# 边迁移表(SOURCE:DEST 空格分隔)。
#   - 主流程 14 步:INIT → PREFLIGHT_PASSED → ... → MAINTENANCE_DISABLED
#   - 失败兜底:SAFE_STOPPED 由 is_valid_edge 通配分支处理(任意状态可入,见下方)
#   - 恢复流程(Task 6 brief):任何升级状态 → RECOVERY_STARTED(失败兜底入口),
...
#   - RECOVERY_* → SAFE_STOPPED 由"to==SAFE_STOPPED"通配分支处理(无需列在表中)
STATE_EDGES='INIT:PREFLIGHT_PASSED PREFLIGHT_PASSED:MAINTENANCE_ENABLED MAINTENANCE_ENABLED:BACKUP_CREATED BACKUP_CREATED:BACKUP_VERIFIED BACKUP_VERIFIED:RUNTIME_SNAPSHOT_RECORDED RUNTIME_SNAPSHOT_RECORDED:WRITE_SERVICES_STOPPED WRITE_SERVICES_STOPPED:TARGET_IMAGE_READY TARGET_IMAGE_READY:MIGRATION_PREFLIGHT_PASSED MIGRATION_PREFLIGHT_PASSED:MIGRATED MIGRATED:TARGET_HEALTHY TARGET_HEALTHY:SMOKE_TEST_PASSED SMOKE_TEST_PASSED:COMMITTED COMMITTED:MAINTENANCE_DISABLED INIT:RECOVERY_STARTED PREFLIGHT_PASSED:RECOVERY_STARTED MAINTENANCE_ENABLED:RECOVERY_STARTED BACKUP_CREATED:RECOVERY_STARTED BACKUP_VERIFIED:RECOVERY_STARTED RUNTIME_SNAPSHOT_RECORDED:RECOVERY_STARTED WRITE_SERVICES_STOPPED:RECOVERY_STARTED TARGET_IMAGE_READY:RECOVERY_STARTED MIGRATION_PREFLIGHT_PASSED:RECOVERY_STARTED MIGRATED:RECOVERY_STARTED TARGET_HEALTHY:RECOVERY_STARTED SMOKE_TEST_PASSED:RECOVERY_STARTED COMMITTED:RECOVERY_STARTED MAINTENANCE_DISABLED:RECOVERY_STARTED RECOVERY_STARTED:TARGET_SERVICES_STOPPED TARGET_SERVICES_STOPPED:SOURCE_RUNTIME_RESTORED SOURCE_RUNTIME_RESTORED:DATABASE_RESTORED DATABASE_RESTORED:MEDIA_RESTORED MEDIA_RESTORED:RESTORED_STATE_VERIFIED RESTORED_STATE_VERIFIED:SOURCE_HEALTHY SOURCE_HEALTHY:RECOVERY_COMMITTED'
